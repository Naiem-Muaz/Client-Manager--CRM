...
import { Upload, Download, Trash2, Loader2, FileText, Lock } from 'lucide-react';
...
import { SectionCard, EmptyState, TableCard, th, td, btnPrimary } from './ui';
...
export function SponsorDocuments({ workerId }: { workerId: string }) {
  ...
  return (
    <div className="space-y-4">
      <SectionCard icon={Lock} title="Secure document vault" subtitle="Private sponsor-documents store — every access is logged." bodyClass="p-4">
      ...
      {loading ? (
        ...
        <EmptyState icon={FileText} title="No documents uploaded yet" hint="Upload passports, visas, CoS and RTW evidence to the secure vault." />
      ) : (
        <TableCard head={<tr><th className={th}>Type</th><th className={th}>File</th><th className={th}>Uploaded</th><th className={th}></th></tr>}>
          {docs.map(d => (
            <tr key={d.id} className="hover:bg-slate-50">
              <td className={`${td} font-medium text-[#0F1E3A]`}>{typeLabel(d.documentType || d.docType)}</td>
              <td className={`${td} text-slate-600`}><span className="inline-flex items-center gap-2"><FileText size={14} className="text-slate-400" />{d.fileName}{d.sizeBytes ? <span className="text-xs text-slate-400">· {fmtSize(Number(d.sizeBytes))}</span> : null}</span></td>
              <td className={`${td} text-slate-500 text-xs tabular-nums`}>{fmtDate(d.uploadedAt)}{d.uploadedBy ? <span className="text-slate-400"> · by {String(d.uploadedBy).slice(0, 8)}</span> : ''}</td>
