...
export function useInvoices(clientId: string) {
  const { data, error, isLoading } = useSWR(`/api/brain/clients/${clientId}/invoices`, fetcher);
...
export async function createInvoice(clientId: string, invoiceData: Record<string, any>) {
  const response = await NextGenAPI.post(`/api/brain/clients/${clientId}/invoices`, invoiceData);
  mutate(`/api/brain/clients/${clientId}/invoices`);
  return response.data;
}

export async function updateInvoiceStatus(clientId: string, invoiceId: string, status: 'draft' | 'sent' | 'paid') {
  const response = await NextGenAPI.put(`/api/brain/clients/${clientId}/invoices/${invoiceId}/status`, { status });
  mutate(`/api/brain/clients/${clientId}/invoices`);
  return response.data;
}
